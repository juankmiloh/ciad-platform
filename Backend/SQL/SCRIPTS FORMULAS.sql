...
--- TENER EN CUENTA QUE ESTE FORMATO TIENE LOS CAMPOS DE MES Y AÑO CORREGIDO COMO FT7 - validar
------------------------------------------------------------------------------------------
SELECT 
...
(
    SELECT 
        ...
        FECHAS_TIPO_GARANTIA_1, FTE2.TIPO_GARANTIA_1 AS C56, 
        FECHAS_TIPO_GARANTIA_2, FTE2.TIPO_GARANTIA_2 AS C57, 
        FECHAS_TIPO_GARANTIA_3, NVL(FTE2.TIPO_GARANTIA_3, 0) AS C57_1 
    FROM 
    (
...
----------------------  CONSULTAS DE VR Y USUARIOS ---------------

SELECT * FROM all_tab_columns
WHERE COLUMN_NAME LIKE '%1732%';
...
(
    SELECT 
        TO_NUMBER(SUBSTR(CAR_T1743_MERCADO_NIU, 1, INSTR(CAR_T1743_MERCADO_NIU, '-')-1)) AS MERCADO,
        TO_NUMBER(SUBSTR(CAR_T1743_MERCADO_NIU, INSTR(CAR_T1743_MERCADO_NIU, '-')+1)) AS NIU,
        CAR_T1743_TIPO_TARIFA 
    FROM ENERGIA_CREG_015.CAR_T1743_TC2FACTURACION_USU 
    WHERE 
        IDENTIFICADOR_EMPRESA = :EMPRESA_ARG 
        AND CAR_CARG_PERIODO = :PERIODO_ARG 
...
SELECT
    CAR_T1743_TIPO_TARIFA FROM ENERGIA_CREG_015.CAR_T1743_TC2FACTURACION_USU
WHERE 
    IDENTIFICADOR_EMPRESA = :EMPRESA_ARG
    and CAR_CARG_PERIODO = :PERIODO_ARG
    AND CAR_CARG_ANO = :ANIO_ARG;
